...
def send_vendor_notification(vendor_email, vendor_name, subject, message, action_type):
    ...
    <head>
        ...
        <style>
            ...
            .header {{ background: linear-gradient(135deg, #0000c1, #1a1ae6); padding: 30px; text-align: center; border-radius: 10px 10px 0 0; }}
...
def vendor_withdraw():
    ...
    # Recalculate the wallet from orders first. This also creates the wallet
    # row if it doesn't exist yet (e.g. a vendor who never opened /vendor/wallet),
    # which previously caused a silent 404 "Wallet not found" here.
    sync_vendor_wallet(user_id)
        ...
        try:
            ...
            for field in required_fields:
                if not step1.get(field):
                    return jsonify({
                        'success': False,
                        'message': f'Missing {field}. Please go back to Step 1.'
                    }), 400
...
def send_purchase_confirmation(email, full_name, item_title, item_type, item_id):
    ...
    <head>
        ...
        <style>
            body {{ font-family: Arial, sans-serif; line-height: 1.6; color: #333; }}
            .container {{ max-width: 600px; margin: 0 auto; padding: 20px; }}
            .header {{ background: linear-gradient(135deg, #0000c1, #00008f); padding: 30px; text-align: center; border-radius: 10px 10px 0 0; }}
...
def send_verification_email(email, full_name, verification_token, verification_code=None):
    ...
    <head>
        ...
        <style>
            ...
            .footer {{ text-align: center; margin-top: 20px; color: #888; font-size: 12px; }}
            .divider {{ border: none; border-top: 1px solid #e1e5ee; margin: 24px 0; }}
        </style>
    </head>
    <body>
        <div class="container">
    ...
    <head>
        ...
        <style>
            ...
            .btn {{
                ...
                font-weight: 600;
                margin: 20px 0;
                box-shadow: 0 4px 15px rgba(0, 0, 193, 0.3);
            }}
            .btn:hover {{ box-shadow: 0 6px 25px rgba(0, 0, 193, 0.4); }}
            .warning-box {{
                background: #fef3c7;
